 ...
 */
include_once dirname(__FILE__) . '/../config/_config.php';
include_once dirname(__FILE__) . '/../config/_statusCodes.php';
...
class Utils {
    ...
    public static function PDOFetchRecords($tableName, $columnsToFetch = array(), $filters = array(), $dbConn = null, $fetchMode = PDO::FETCH_ASSOC) {
        ...
        if ($dbConn == null) {
            return Utils::formatResponse(null, SC_FAILURE_CODE, 4, 'THERE WAS AN ERROR CONNECTING TO THE DATABASE');
        }
    ...
    public static function executePreparedStatement($SQL, $params, $fetchMode = null, $dbConn = null, $noFetch = false, $getLastInsert = false) {
        ...
        try {
            ...
            return Utils::formatResponse(null, SC_FAILURE_CODE, 4, $exc->getMessage());
        } catch (Exception $e) {
            Utils::log('EXCEPTION', 'A DATABASE ERROR OCCURRED | ' . $e->getCode() . ' | ' . $e->getMessage(), __CLASS__, __FUNCTION__, __LINE__);
            return Utils::formatResponse(null, SC_FAILURE_CODE, 4, $e->getMessage());
        }
        //return data

        return Utils::formatResponse($results, SC_SUCCESS_CODE, 1, SC_SUCCESS_DESC);
    }
    ...
    public static function setSessionMessage($type = null, $message = null){
        if (session_status() == PHP_SESSION_NONE) {
            session_start();
        }
        $_SESSION['message']['type'] = ($type == null) ? 'info' : $type;
        $_SESSION['message']['content'] = ($message == null) ? 'info' : $message;
    }

    public static function showSessionMessage($type = null){
        //check if any session messages have been set
        if(!isset($_SESSION['message'])){
            Utils::log(DEBUG, 'message not set in session', __CLASS__, __FUNCTION__, __LINE__);
            return;
        }
        $sessionMessages = $_SESSION['message'];
        $message_type = (isset($sessionMessages['type'])) ? $sessionMessages['type'] : 'info';
        $message = (isset($sessionMessages['content'])) ? $sessionMessages['content'] : 'An error occurred. Please try again';
        echo '<div class="alert alert-'.$message_type.'  alert-dismissable">'
